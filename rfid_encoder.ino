...
void loop()
{
  ...
  mfrc522.PICC_DumpToSerial(&(mfrc522.uid));//blocks further authenitcation
  
  mfrc522.PICC_HaltA(); // Halt PICC
  mfrc522.PCD_StopCrypto1();  // Stop encryption on PCD
}

void DrawerEncode(byte num,byte* out_encoded){
  if(num>255){
    for(int i=0;i<6;i++)
      out_encoded[i]=0xFF;
    return;
